...
import { 
  MapPin, Users, Settings, Plus, Edit2, Trash2, 
  Loader2, CheckCircle2, FileSpreadsheet, ExternalLink,
  Building2, Globe
} from "lucide-react";
...
const AdminPanel = () => {
  ...
  const [siteForm, setSiteForm] = useState({ name: "", address: "", latitude: "", longitude: "", radius_meters: 200 });
  ...
  const openSiteDialog = (site = null) => {
    if (site) {
      ...
      setEditingSite(null);
      setSiteForm({ name: "", address: "", latitude: "", longitude: "", radius_meters: 200 });
    }
    setSiteDialogOpen(true);
  };

  const handleSaveSite = async () => {
    if (!siteForm.name || !siteForm.address || !siteForm.latitude || !siteForm.longitude) {
  ...
  const handleConnectSheets = async () => {
    try {
      ...
      toast.error(error.response?.data?.detail || "Failed to initiate Google Sheets connection");
    }
  };
    ...
    <Layout>
      <div className="max-w-6xl mx-auto p-4 space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Manage sites, users, and integrations</p>
        </div>
        ...
        <Tabs defaultValue="sites" className="w-full">
          ...
          <TabsContent value="sites" className="mt-6 space-y-4">
            <div className="flex justify-between items-center">
              <p className="text-muted-foreground">{sites.length} work sites configured</p>
              <Button onClick={() => openSiteDialog()} data-testid="add-site-btn">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground line-clamp-1">{site.address}</p>
                        </div>
                      </div>
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" onClick={() => openSiteDialog(site)}>
                          <Edit2 className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteSite(site.site_id)}>
            ...
            <Dialog open={siteDialogOpen} onOpenChange={setSiteDialogOpen}>
              <DialogContent className="bg-card border-border">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-sm text-muted-foreground">Geo-fence Radius (meters)</label>
                    <Input
                      data-testid="site-radius-input"
                      type="number"
                      value={siteForm.radius_meters}
                      onChange={(e) => setSiteForm({ ...siteForm, radius_meters: e.target.value })}
                      className="bg-background mt-1 font-mono"
                    />
                  </div>
                </div>
                <DialogFooter>
                  ...
                  <Button onClick={handleSaveSite} disabled={savingSite} data-testid="save-site-btn">
                    {savingSite && <Loader2 className="w-4 h-4 animate-spin mr-2" />}
                    Save
                  </Button>
                </DialogFooter>
              </DialogContent>
          ...
          <TabsContent value="users" className="mt-6">
            ...
            <Dialog open={userDialogOpen} onOpenChange={setUserDialogOpen}>
              <DialogContent className="bg-card border-border">
                <DialogHeader>
                  <DialogTitle className="font-heading">Edit User: {editingUser?.name}</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                    <label className="text-sm text-muted-foreground">Role</label>
                    <Select value={userForm.role} onValueChange={(v) => setUserForm({ ...userForm, role: v })}>
                      <SelectTrigger className="bg-background mt-1" data-testid="user-role-select">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    <label className="text-sm text-muted-foreground">Numeric ID (for quick login)</label>
                    <Input
                    ...
                    <div className="mt-2 space-y-2 max-h-48 overflow-y-auto">
                      {sites.map((site) => (
                        <label key={site.site_id} className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={userForm.assigned_sites.includes(site.site_id)}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setUserForm({ ...userForm, assigned_sites: [...userForm.assigned_sites, site.site_id] });
                              } else {
                                setUserForm({ ...userForm, assigned_sites: userForm.assigned_sites.filter(s => s !== site.site_id) });
                              }
                            }}
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleSaveUser} disabled={savingUser} data-testid="save-user-btn">
          ...
          <TabsContent value="integrations" className="mt-6 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <p className="text-sm text-muted-foreground">
                      Sync approved timesheets directly to Google Sheets for payroll processing.
                    </p>
                  ...
                  {sheetsStatus?.connected ? (
                    ...
                    <Button onClick={handleConnectSheets} data-testid="connect-sheets-btn">
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  CSV export is always available as a backup. Employees and managers can export their 
                  timesheets directly from the Timesheet page.
                </p>
